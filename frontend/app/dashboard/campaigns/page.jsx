...
export default function CampaignsPage() {
  ...
  const handleStartCampaign = async (id) => {
    try {
      ...
      if (data.success) {
        toast.success(data.message || 'Campaign blast queued!');
        fetchCampaigns();
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-wa-text-primary dark:text-wa-dark-text-primary">Blast Campaigns</h2>
          <p className="text-sm text-wa-text-secondary dark:text-wa-dark-text-secondary mt-0.5">Launch marketing campaigns and track delivery metrics.</p>
        </div>
        <button
      ...
      <div className="glass-card overflow-hidden">
        <div className="px-6 py-4 border-b border-wa-border dark:border-wa-dark-border flex justify-between items-center bg-wa-panel-header/50 dark:bg-wa-dark-panel-header/50">
          <h3 className="font-semibold text-sm text-wa-text-primary dark:text-wa-dark-text-primary">Campaigns Log</h3>
          <button
              ...
              {loading && campaigns.length === 0 ? (
                ...
                <tr>
                  <td colSpan="7" className="px-6 py-16 text-center text-wa-text-secondary">
                    ...
                    <p className="text-xs mt-1 opacity-70">Click "New Campaign" to start.</p>
                  </td>
                </tr>
              ) : (
